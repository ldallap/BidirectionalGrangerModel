...
/*RECEVEIVER POPULATION = POPULATION 1, gamma oscillations*/
/*SENDER POPULATION = POPULATION 2, alpha oscilaltions*/

#define dt 0.05 /*integration time step (ms)*/
#define gES 0.8 /*excitatory conductance within SENDER (ns)*/
#define gIS 16.4 /*inhibitory conductance within SENDER (ns)*/

#define gER 3.0 /*excitatory conductance within RECEIVER (ns)*/
#define gIR 16.0 /*excitatory conductance within RECEIVER (ns)*/

#define gSR 4.0 /*excitatory conductance from SENDER to RECEIVER (ns)*/
#define gRS 0.15 /*excitatory conducatence from RECEIVER to SENDER (ns)*/


#define gEPoisson 0.6 /*excitatory Poisson conductance (ns)*/
#define totaltime 1000000 /*total number of steps of simulation*/
#define transient 40000 /*steps to discard before save data*/


float ran2(long *idum); /*call the random number generator described in the end*/
long seed;
...
int main(int argc, char *argv[]){
  ...
  seed= (long) atoi(argv[1]); /*allocate the random seed number*/
  /**/

  /*Create datafile names*/
  FILE *outf;
  FILE *outf2;
  ...
  /*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
  /*&&                    CONNECTIVITY MATRIX                   &&*/
  /*&& indexes from 0-399 stand for EXC neurons, 400-499 for INH neurons &&*/
  ...
  /*&&*/  }
  /*&&*/}
  /*&&*/for(int j=0;j<500;j++){ /*20 random exc connec from Sender to Receiver and back*/
  ...
  /*&&*/for(int i=0;i<100;i++){
  /*&&*/	monitor_neurons[i]=(int)(400.0*ran2(&seed));
  /*&&*/	for(int j=0;j<i;j++){
  /*&&*/		do monitor_neurons[i]=(int)(400.0*ran2(&seed));
  ...
  for(int t=0;t<totaltime;t++){ /*Looping over time*/
    ...
    /*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
  	if(t*dt*0.001>=9 and t*dt*0.001<20){
	  	//*SPIKE TRAINS*//
	    for(int j=0;j<400;j++){
	    	for(int k=0;k<100;k++){
